...
def adamwh(params: List[Tensor],
    ...
    if foreach and not torch.jit.is_scripting():
        func = _multi_tensor_adamwh
    else:
        func = _single_tensor_adamwh
...
def _single_tensor_adamwh(params: List[Tensor],
                         grads: List[Tensor],
                         exp_avgs: List[Tensor],
                         exp_avg_sqs: List[Tensor],
                         max_exp_avg_sqs: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if capturable:
            ...
            ### OUR CHANGING!!!
            grad_norm[0] = torch.norm(exp_avg, p=2)
            grad_norm[1] = torch.norm(exp_avg/bias_correction1 + weight_decay * param * denom , p=2)
...
def _multi_tensor_adamwh(params: List[Tensor],
                        grads: List[Tensor],
                        exp_avgs: List[Tensor],
                        exp_avg_sqs: List[Tensor],
                        max_exp_avg_sqs: List[Tensor],
    ...
    if capturable:
        ...
        if amsgrad:
            ...
            denom = torch._foreach_maximum_(exp_avg_sq_sqrt, weight_decay)
